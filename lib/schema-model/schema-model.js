 ...
 *   @param {QueryFactory} options.queryFactory
 *   @param {UpdateFactory} options.updateFactory
 *   @param {AggregateFactory} options.aggregateFactory
 */
class SchemaModel extends Model {
	...
	constructor(schema, options = {}) {
		...
		super(options);
		this.queryFactory = options.queryFactory || commonQuery.defaultQueryFactory;
		this.updateFactory = options.updateFactory || commonQuery.defaultUpdateFactory;
		this.aggregateFactory = options.aggregateFactory || commonQuery.defaultAggregateFactory;
	...
	normalizeQuery(query, options = {}) {
		...
		if (_.isPlainObject(query)) {
			query = this.queryFactory.createQuery(query, normalizeOptions);
		} else {
	...
	normalizeUpdate(update, options = {}) {
		...
		if (_.isPlainObject(update)) {
			update = this.updateFactory.createUpdate(update, normalizeOptions);
		} else {
	...
	normalizeAggregate(aggregate, options = {}) {
		...
		if (_.isPlainObject(aggregate)) {
			aggregate = this.aggregateFactory.createAggregate(aggregate, normalizeOptions);
		} else {
