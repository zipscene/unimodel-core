...
class Model extends CrispPrePostHooks {
	 ...
	 * @return {DocumentStream} - A readable object stream streaming document instances.
	 *   The stream also contains a method called `getTotal()`,
	 *   which returns a promise resolving to the total number of results.
	 */
	 ...
	 * @param {Object} query
	 * @param {Object} [options] - See the options for `find()`.
	 *   Options `total` and `limit` are not supported.
	 * @return {Promise} - Resolves with the single document instance if found.
	 *   On not found, rejects with an XError with the code `XError.NOT_FOUND` .
	 *   On error, rejects with an XError.
	 */
	findOne(query, options = {}) {
		...
		return this.find(query, options)
			.then((results) => {
				if (!_.isArray(results)) throw new XError(XError.UNSUPPORTED_FORMAT, 'Unexpected value was returned');
				if (results.length === 0) throw new XError(XError.NOT_FOUND, 'No matching document was found.');
	 ...
	 * @return {Promise} - May resolve with `undefined`, or, if a new document was inserted,
	 *   may resolve like `insert()` .
	 */
	 ...
	 * @return {Promise} - May resolve with either:
	 *   - undefined
	 *   - A subset of the inserted document data, including, at minimum, any fields
	 *     added by the database layer (such as a generated ID).
	 */
	insert(data, options = {}) {
	 ...
	 * @param {Object} [options] - Model-specific options.
	 * @return {Promise} - Resolves with an array of result objects, as from
	 *   `insert()`.
	 */
