...
class Model extends CrispPrePostHooks {
	...
	/**
	 * Returns a list of key fields used to uniquely identify documents in this collection.  Keys
	 * should be listed in order from most-specific to least-specific.  For example, in a collection
	 * of cities, the keys might be: `[ 'cityName', 'state', 'country', 'planet' ]`.
	 *
	 * @method getKeys
	 * @since v0.0.1
	 ...
	 * @return {Promise{Document}} - Resolves with the Document representation or rejects with XError.
	 */
	insert(data, options = {}) {
	 ...
	 * @param {Object} [options] - Model-specific options.
	 * @return {Promise{Array{Document}}} - Resolves with the Document representation or rejects with XError.
	 */
	insertMulti(datas, options = {}) {
		...
		return pasync.map(datas, (data) => this.insert(data, options));
	}
